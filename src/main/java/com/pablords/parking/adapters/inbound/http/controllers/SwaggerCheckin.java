...
public interface SwaggerCheckin {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = Constants.BAD_REQUEST, description = "Requisição inválida", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = ApiError.class), examples = @ExampleObject(value = "{\"status\":400,\"error\":\"BadRequest\",\"message\":\"Invalid checkin,checkout notfound\",\"path\":\"/api/v1/checkins\",\"timestamp\":\"2025-02-02T09:55:23\"}")) }),
